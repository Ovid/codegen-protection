...
sub BUILD {
    ...
    if ( $self->has_old_text ) {
        ...
        my $new_text = $self->new_text;
        $new_text
          = $self->_remove_all_leading_and_trailing_blank_lines($new_text);
        $self->_set_rewritten( $self->add_checksums($new_text) );
    }
}
...
sub _rewrite {
    ...
    my $body = $self->add_checksums($replacement);
...
sub _regex_to_match_rewritten_document {
    ...
    my $start_marker_re
      = sprintf $class->_start_marker_format => $class->VERSION,
      $digest_start_re;
    my $end_marker_re = sprintf $class->_end_marker_format => $class->VERSION,
      $digest_end_re;
...
=head2 C<add_checksums>
    ...
    my $checksummed_text = Data::Rewrite->add_checksums($text);
...
it's _remove_all_leading_and_trailing_blank_linesmed).
